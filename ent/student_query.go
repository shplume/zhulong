...
func (sq *StudentQuery) QueryUserID() *UserQuery {
...
func (sq *StudentQuery) WithUserID(opts ...func(*UserQuery)) *StudentQuery {
	query := (&UserClient{config: sq.config}).Query()
...
func (sq *StudentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Student, error) {
	...
	if query := sq.withUserID; query != nil {
		if err := sq.loadUserID(ctx, query, nodes, nil,
			func(n *Student, e *User) { n.Edges.UserID = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *StudentQuery) loadUserID(ctx context.Context, query *UserQuery, nodes []*Student, init func(*Student), assign func(*Student, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Student)
	for i := range nodes {
		if nodes[i].student_user_id == nil {
			continue
		}
		fk := *nodes[i].student_user_id
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "student_user_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
