...
type ReviewQuery struct {
	...
	withThesis    *FileQuery
	withStudentID *StudentQuery
	withTeacherID *TeacherQuery
	withBookID    *FileQuery
	withFKs       bool
...
func (rq *ReviewQuery) QueryThesis() *FileQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, review.ThesisTable, review.ThesisColumn),
		)
...
// QueryStudentID chains the current query on the "student_id" edge.
func (rq *ReviewQuery) QueryStudentID() *StudentQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, review.StudentIDTable, review.StudentIDColumn),
		)
		fromU = sqlgraph.SetNeighbors(rq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (rq *ReviewQuery) QueryTeacherID() *TeacherQuery {
	query := (&TeacherClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, review.TeacherIDTable, review.TeacherIDColumn),
		)
		fromU = sqlgraph.SetNeighbors(rq.driver.Dialect(), step)
		return fromU, nil
	}
...
// QueryBookID chains the current query on the "book_id" edge.
func (rq *ReviewQuery) QueryBookID() *FileQuery {
	query := (&FileClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(review.Table, review.FieldID, selector),
			sqlgraph.To(file.Table, file.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, false, review.BookIDTable, review.BookIDColumn),
		)
		fromU = sqlgraph.SetNeighbors(rq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (rq *ReviewQuery) Clone() *ReviewQuery {
	...
	return &ReviewQuery{
		...
		withThesis:    rq.withThesis.Clone(),
		withStudentID: rq.withStudentID.Clone(),
		withTeacherID: rq.withTeacherID.Clone(),
		withBookID:    rq.withBookID.Clone(),
...
func (rq *ReviewQuery) WithThesis(opts ...func(*FileQuery)) *ReviewQuery {
	query := (&FileClient{config: rq.config}).Query()
...
// WithStudentID tells the query-builder to eager-load the nodes that are connected to
// the "student_id" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *ReviewQuery) WithStudentID(opts ...func(*StudentQuery)) *ReviewQuery {
	query := (&StudentClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withStudentID = query
	return rq
}

// WithTeacherID tells the query-builder to eager-load the nodes that are connected to
// the "teacher_id" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *ReviewQuery) WithTeacherID(opts ...func(*TeacherQuery)) *ReviewQuery {
	query := (&TeacherClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withTeacherID = query
	return rq
}

// WithBookID tells the query-builder to eager-load the nodes that are connected to
// the "book_id" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *ReviewQuery) WithBookID(opts ...func(*FileQuery)) *ReviewQuery {
	query := (&FileClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *ReviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Review, error) {
	var (
		...
		loadedTypes = [4]bool{
			rq.withThesis != nil,
			rq.withStudentID != nil,
			rq.withTeacherID != nil,
			rq.withBookID != nil,
		}
	)
	if rq.withThesis != nil || rq.withStudentID != nil || rq.withTeacherID != nil || rq.withBookID != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withThesis; query != nil {
		if err := rq.loadThesis(ctx, query, nodes, nil,
			func(n *Review, e *File) { n.Edges.Thesis = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withStudentID; query != nil {
		if err := rq.loadStudentID(ctx, query, nodes, nil,
			func(n *Review, e *Student) { n.Edges.StudentID = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withTeacherID; query != nil {
		if err := rq.loadTeacherID(ctx, query, nodes, nil,
			func(n *Review, e *Teacher) { n.Edges.TeacherID = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withBookID; query != nil {
		if err := rq.loadBookID(ctx, query, nodes, nil,
			func(n *Review, e *File) { n.Edges.BookID = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReviewQuery) loadThesis(ctx context.Context, query *FileQuery, nodes []*Review, init func(*Review), assign func(*Review, *File)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Review)
	for i := range nodes {
		if nodes[i].review_thesis == nil {
			continue
		}
		fk := *nodes[i].review_thesis
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "review_thesis" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ReviewQuery) loadStudentID(ctx context.Context, query *StudentQuery, nodes []*Review, init func(*Review), assign func(*Review, *Student)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Review)
	for i := range nodes {
		if nodes[i].review_student_id == nil {
			continue
		}
		fk := *nodes[i].review_student_id
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "review_student_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ReviewQuery) loadTeacherID(ctx context.Context, query *TeacherQuery, nodes []*Review, init func(*Review), assign func(*Review, *Teacher)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Review)
	for i := range nodes {
		if nodes[i].review_teacher_id == nil {
			continue
		}
		fk := *nodes[i].review_teacher_id
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "review_teacher_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ReviewQuery) loadBookID(ctx context.Context, query *FileQuery, nodes []*Review, init func(*Review), assign func(*Review, *File)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Review)
	for i := range nodes {
		if nodes[i].review_book_id == nil {
			continue
		}
		fk := *nodes[i].review_book_id
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "review_book_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
