...
func (tq *TeacherQuery) QueryUserID() *UserQuery {
...
func (tq *TeacherQuery) WithUserID(opts ...func(*UserQuery)) *TeacherQuery {
	query := (&UserClient{config: tq.config}).Query()
...
func (tq *TeacherQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Teacher, error) {
	...
	if query := tq.withUserID; query != nil {
		if err := tq.loadUserID(ctx, query, nodes, nil,
			func(n *Teacher, e *User) { n.Edges.UserID = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TeacherQuery) loadUserID(ctx context.Context, query *UserQuery, nodes []*Teacher, init func(*Teacher), assign func(*Teacher, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Teacher)
	for i := range nodes {
		if nodes[i].teacher_user_id == nil {
			continue
		}
		fk := *nodes[i].teacher_user_id
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "teacher_user_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
