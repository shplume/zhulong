...
func (fq *FileQuery) QueryUserID() *UserQuery {
...
func (fq *FileQuery) Clone() *FileQuery {
	...
	return &FileQuery{
		...
		withUserID: fq.withUserID.Clone(),
...
func (fq *FileQuery) WithUserID(opts ...func(*UserQuery)) *FileQuery {
	query := (&UserClient{config: fq.config}).Query()
...
func (fq *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := fq.withUserID; query != nil {
		if err := fq.loadUserID(ctx, query, nodes, nil,
			func(n *File, e *User) { n.Edges.UserID = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FileQuery) loadUserID(ctx context.Context, query *UserQuery, nodes []*File, init func(*File), assign func(*File, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*File)
	for i := range nodes {
		if nodes[i].file_user_id == nil {
			continue
		}
		fk := *nodes[i].file_user_id
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "file_user_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
